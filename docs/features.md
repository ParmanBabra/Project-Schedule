...
- **Buffer ถูกล็อกที่ baseline**: เมื่อมี baseline ขนาดเผื่อ (`bufferDays`) และวันสัญญาส่ง (`buffer.end`) จะคงที่ ส่วนแถบเผื่อวาดจากวันเสร็จตามแผน *ปัจจุบัน* ถึงวันสัญญาส่ง: เสร็จเร็วกว่า baseline แถบยาวขึ้นและชิปบอก "ล่วงหน้า N วัน" (`aheadDays`) เลื่อนช้าแถบสั้นลงและนับเป็น "ใช้ไป %" เกินสัญญาส่งแถบหายและ % ≥ 100 วันสัญญาส่งจึงไม่ขยับตามแผนที่เลื่อน การเปลี่ยนวิธี buffer มีผลเมื่อบันทึก baseline ใหม่เท่านั้น · UI: ชิป "เผื่อ N วัน" บน Gantt มีไอคอนล็อกและ tooltip อธิบาย (สายงานหลักตอน baseline เทียบตอนนี้) และหน้าตั้งค่าเตือนเมื่อสายงานหลักเปลี่ยนเกิน 25% จาก baseline พร้อมปุ่ม "บันทึกใหม่จากแผนปัจจุบัน"
