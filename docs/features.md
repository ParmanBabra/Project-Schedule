...
- **Baseline ล็อก "วันสัญญาส่ง" ไม่ใช่ตัวเผื่อ**: เผื่อคำนวณจากสายงานหลักปัจจุบันเสมอ (เช่น 50% ของ chain ตอนนี้) และวาดต่อท้ายงานสุดท้าย ส่วน `buffer.committedEnd` (= plannedEnd + bufferDays ตอน baseline) คือคำสัญญาที่ไม่ขยับ: เสร็จเร็วกว่า baseline → `aheadDays` และมีเส้นประจากปลายเผื่อถึงเพชรสัญญาส่ง · เลื่อนช้ากว่า baseline → `consumedPercent` เทียบกับขนาดเผื่อตอน baseline ขับสัญญาณเขียว/เหลือง/แดง · การเปลี่ยนวิธี/สัดส่วนเผื่อมีผลทันทีกับแถบ แต่คำสัญญาเปลี่ยนเมื่อบันทึก baseline ใหม่เท่านั้น
