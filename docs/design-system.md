...
| `--text-3` | `#716a91` | ป้ายกำกับ หัวตาราง (ปรับจาก #8a83a8 ให้ผ่าน AA 4.5:1 ใน Phase 7) |
...
| `--w-namecol` | 220px คอลัมน์ชื่องาน (มือถือ 120px) ค่าเริ่มต้น; Gantt ทับด้วยค่าที่ผู้ใช้ลากปรับ 120 – 560px |
...
## DatePicker (เพิ่ม 2026-09-17)

แทนที่ `<input type="date">` ทุกจุด เพราะรูปแบบวันที่ของ native ขึ้นกับภาษาเครื่อง (เช่น 14-Sep-2026)

- ปุ่มควบคุมสูง `--h-input` พื้น `--surface-3` มุม `--r-input` ไอคอนปฏิทิน + วันที่แบบไทย พ.ศ. ("14 ก.ย. 2569") ว่าง = "เลือกวันที่" สี `--text-3` ปุ่ม × ล้างค่าเมื่อ `clearable`
- Desktop: popover กว้าง 300px ใต้ปุ่ม มุม `--r-card` เงา `--sh-float` หัวเดือน "กันยายน 2569" พร้อม ‹ › แถววันจ.–อา. ช่องวัน 38px มุม 10px วันที่เลือกพื้น `--primary` วันนี้ขอบใน `--primary-tint` เสาร์–อาทิตย์สี `--text-3` นอกเดือน `--text-4` นอกช่วง min/max จาง 30% ท้ายมี "ไปเดือนนี้" และ "วันนี้ · 15 ก.ย."
- มือถือ (< 768px): bottom sheet เต็มกว้าง มุม `--r-sheet` ช่องวัน 44px (พื้นที่แตะ) มีปุ่มปิด และปิดเมื่อแตะฉากหลังหรือ Escape
- ค่าที่ส่ง/รับเป็น ISO `YYYY-MM-DD` เสมอ · ทดสอบใช้ `data-testid="date-picker"` และ `day-<ISO>` (helper `e2e/helpers.ts: pickDate`)
